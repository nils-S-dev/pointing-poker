            ...
            <div className={ `grid gap-3 grid-cols-4 xl:grid-cols-8 md:gap-5 ${ className }` }>
                {
                    options.map((option: EstimationOption) => (
                        <button key={ option.value } onClick={ () => onClick(option.value) } className={ `aspect-square rounded-xl text-xl  border-steel-400 transition-all border shadow-2xl hover:bg-steel-400 hover:scale-105 active:bg-sky-700 ${ estimation === option.value ? 'bg-sky-700' : 'bg-steel-500' }` }>{ option.label }</button>
                    ))
                }
            </div>
        </>
